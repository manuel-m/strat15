(() => {
  ...
  function s15_default(o = {}) {
    ...
    const strats = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m"];
    const strats_data = strats.reduce(function(a, k) {
      a[k] = { players: players.map(Player) };
      return a;
    }, {});
    ...
    m.selected_strat = m.strats[0];
    return m;
    function strat_next() {
      const idx = strats.indexOf(m.selected_strat);
      return idx < strats.length - 1 ? strats[idx + 1] : m.selected_strat;
    }
    function strat_prev() {
      const idx = strats.indexOf(m.selected_strat);
      return idx > 0 ? strats[idx - 1] : m.selected_strat;
    }
    function player_get(player_id) {
      return m.strats_data[m.selected_strat].players.find(
        (o2) => o2.id === player_id
      );
    }
    function Player(id) {
  ...
  function S15_ui(s15) {
    ...
    registerActions();
    function clearStart() {
      for (const p of s15.strats_data[s15.selected_strat].players) {
        p.x = 0;
        p.y = 0;
      }
      const els = document.querySelectorAll(".s15-draggable");
      for (const el of els) {
        el.setAttribute("data-x", 0);
        el.setAttribute("data-y", 0);
        el.style = "translate(" + 0 + "px, " + 0 + "px)";
      }
    }
    function copyStrat() {
      const toStrat_id = s15.strat_next();
      if (toStrat_id === s15.selected_strat) {
        return;
      }
      const players_dom = players_fromDom();
      s15.strats_data[s15.selected_strat].players = players_dom;
      s15.strats_data[toStrat_id].players = JSON.parse(
        JSON.stringify(players_dom)
      );
      toStrat(toStrat_id);
    }
    function toStrat(toStrat_id) {
      if (toStrat_id === s15.selected_strat) {
        console.warn("no change");
        return;
      }
      const prev = document.querySelector(".s15-strat-selector.selected");
      if (prev) {
        prev.classList.remove("selected");
      }
      const players_dom = players_fromDom();
      s15.strats_data[s15.selected_strat].players = players_dom;
      const players_wanted = s15.strats_data[toStrat_id].players;
      for (const wanted of players_wanted) {
        const tgt = document.getElementById("s15-player-" + wanted.id);
        tgt.style.transform = "translate(" + wanted.x + "px, " + wanted.y + "px)";
        tgt.setAttribute("data-x", wanted.x);
        tgt.setAttribute("data-y", wanted.y);
      }
      s15.selected_strat = toStrat_id;
      const target = document.getElementById(
        "s15-strat-selector-" + s15.selected_strat
      );
      target.classList.add("selected");
    }
    function players_fromDom() {
      const els = document.querySelectorAll(".s15-draggable");
      const out = [];
      for (const el of els) {
        out.push({
    ...
    function registerActions() {
      for (const el of document.querySelectorAll("[data-action]")) {
        el.addEventListener("click", (evt) => {
          actions[el.getAttribute("data-action")](el);
        });
      }
    }
    function enableDragDrop() {
      (0, import_interactjs.default)(".s15-draggable").draggable({
        inertia: true,
        modifiers: [
    ...
    function genLeftHtml() {
      const el = document.getElementById("s15-left-wrapper");
      el.innerHTML = s15.strats.map(function(c) {
        const item_class = "s15-strat-selector" + (s15.selected_strat === c ? " selected" : "");
        return `<div class="${item_class}" id="s15-strat-selector-${c}">${c}</div>`;
      }).join("");
      for (const el2 of document.querySelectorAll(".s15-strat-selector")) {
        el2.addEventListener("click", (event) => {
          const toStrat_id = event.target.id.split("-").pop();
          toStrat(toStrat_id);
        });
      }
    }
    function genFieldSvg() {
      const el = document.getElementById("s15-field-wrapper");
      const parentWidth = el.getBoundingClientRect().width;
      const field_w = 1100;
      const field_h = 585;
      const w = field_w;
      const h = 700;
      const r = parentWidth / w;
      el.innerHTML = [
        `<svg width="${w * r}" height="${h * r}" viewBox="0 0 ${w * r} ${h * r}" 
    xmlns="http://www.w3.org/2000/svg">`,
        `<rect x="${3 * r}" y="${4 * r}" width="${1094 * r}" height="${(field_h - 7) * r}" fill="rgb(47, 122, 63)" stroke="#FEFEFE" stroke-width="6"/>
   <line x1="${82 * r}" y1="1" x2="${84 * r}" y2="${field_h * r}" stroke="white" stroke-width="4"/>
   <line x1="${552 * r}" y1="1" x2="${554 * r}" y2="${field_h * r}" stroke="white" stroke-width="4"/>
   <line x1="${203 * r}" y1="1" x2="${205 * r}" y2="${field_h * r}" stroke="white" stroke-width="2"/>
   <line x1="${899 * r}" y1="1" x2="${901 * r}" y2="${field_h * r}" stroke="white" stroke-width="2"/>
   <line x1="${(field_w - 82) * r}" y1="1" x2="${(field_w - 80) * r}" y2="${field_h * r}" stroke="white" stroke-width="4"/>`,
        `<rect x="${2 * r}" y="${(6 + field_h) * r}" width="${(field_w - 4) * r}" height="${64 * r}" fill="#555555" stroke="#777777" stroke-width="4"/>`,
        ...s15.players.map(function(player_id, i) {
          const ps = 48 * r;
          const px = 8 * r;
          const py = (14 + field_h) * r;
          const px_offset = i * ps * 1.4;
          const player = s15.player_get(player_id);
          return player_id === "0" ? [
            `<g id="s15-player-${player_id}" class="s15-draggable" data-x="${player.x}" data-y="${player.y}">`,
            `<text class="s15-ball" x="${px + px_offset + ps / 2 - 1}" y="${py + ps / 2}" text-anchor="middle" dominant-baseline="middle">\u{1F3C9}</text>`,
            "</g>"
          ].join("\n") : [
            `<g id="s15-player-${player_id}" class="s15-draggable" data-x="${player.x}" data-y="${player.y}">`,
            `<text class="s15-player" x="${px + px_offset + ps / 2 - 1}" y="${py + ps / 2}" text-anchor="middle" dominant-baseline="middle">\u{1F455}</text>`,
            `<rect x="${px_offset + ps / 2 + 1} " y="${py + ps / 3}" width="${ps / 4}" height="${ps / 4}" fill="#ddd" />`,
            `<text x="${px + px_offset + ps / 2 - 1}" y="${py + ps / 2}" text-anchor="middle" dominant-baseline="middle" fill="#000" class="s15-txt" >${player_id}</text>`,
            "</g>"
          ].join("\n");
        }),
        "</svg>"
